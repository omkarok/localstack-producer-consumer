#!/bin/bash
for i in {1..5}; do awslocal lambda invoke --endpoint http://localhost:4566 --function-name ToDoCrudStack-lambda-9d4b4c06 --payload '{"title":"'$i'","task":"Automated Tests"}' /tmp/output.txt --region us-east-1; done
